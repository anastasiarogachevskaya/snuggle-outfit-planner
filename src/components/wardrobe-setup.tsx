...
/**
 * The quick / detailed wardrobe setup UI, shared by the signed-in onboarding
 * and the local-first (no account) setup on iPhone so both flows stay
 * identical. Persistence is the caller's job.
 */
export type WardrobeSetupMode = "chooser" | "quick" | "detailed";

export function WardrobeSetup({
  initialSelected,
  saving = false,
  onSave,
  onSkip,
}: {
  initialSelected?: Iterable<WardrobeSlug>;
  saving?: boolean;
  onSave: (slugs: WardrobeSlug[]) => void;
  onSkip: () => void;
}) {
  ...
  const chooseMode = (next: Exclude<WardrobeSetupMode, "chooser">) => {

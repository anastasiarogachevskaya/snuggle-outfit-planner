...
if (!existsSync(storyboardPath)) {
  ...
  if (!/identifier="LaunchLogo"/.test(xml)) {
    errors.push(
      'LaunchScreen.storyboard logo image view is missing accessibilityIdentifier="LaunchLogo".',
    );
  }
  ...
  if (!bg) {
    ...
    for (const [channel, expected] of Object.entries(BRAND_RGB)) {
      ...
      if (!Number.isFinite(value) || Math.abs(value - expected) > 0.002) {
        errors.push(
          `LaunchScreen background ${channel} is ${found ? found[1] : "missing"}; expected #A8B894 (${expected.toFixed(4)}).`,
        );
      }
    }
  }
...
if (existsSync(join(assetsPath, "Splash.imageset"))) {
  errors.push(
    "Splash.imageset has reappeared in Assets.xcassets; the Capacitor splash artwork must stay removed.",
  );
}
  ...
  if (contents) {
    ...
    for (const image of images) {
      ...
      if (!existsSync(join(launchLogoPath, image.filename))) {
        errors.push(
          `LaunchLogo.imageset declares "${image.filename}" but the file is missing on disk.`,
        );
      }
    }
...
console.log(
  "Launch screen asset check passed: branded LaunchScreen + LaunchLogo 1x/2x/3x present, no Capacitor splash.",
);
